 ...
 *   • Support Assistant   → POST /support/query
...
export function createSpeedDial ()
{
    ...
    const styleMiniFab = btn => Object.assign(btn.style, {
        ...
        cursor: 'pointer', background: '#4A5568', color: '#fff',
    ...
    chatBtn.setAttribute('aria-label', 'Open Question Assistant'); chatBtn.title = 'Open Question Assistant';
    chatBtn.innerHTML = SVG_CHAT_ICON; styleMiniFab(chatBtn);
    decorateHover(chatBtn);
    ...
    supportBtn.setAttribute('aria-label', 'Open Support'); supportBtn.title = 'Open Support Assistant';
    supportBtn.innerHTML = SVG_SUPPORT_ICON; styleMiniFab(supportBtn);
    decorateHover(supportBtn);
    ...
    refreshBtn.setAttribute('aria-label', 'Refresh tooltips'); refreshBtn.title = 'Refresh all tooltips';
    refreshBtn.innerHTML = SVG_REFRESH_ICON; styleMiniFab(refreshBtn);
    decorateHover(refreshBtn);
    ...
    Object.assign(mainBtn.style, {
        ...
        cursor: 'pointer', background: '#1A202C', color: '#fff',
    ...
    } catch { try { mainImg.src = chrome.runtime.getURL('icon128.png'); } catch { } }
    ...
    localSpeedDialRef = {
        speedDial, mainButton: mainBtn, actionsContainer: actions,
        refreshButton: refreshBtn, chatButton: chatBtn, supportButton: supportBtn,
        mainImg
    };
...
function createChatModal ()
{
    ...
    Object.assign(modal.style, {
        ...
        width: '330px', height: '480px', background: '#fff', borderRadius: '16px',
    ...
    Object.assign(header.style, {
        padding: '16px 20px', background: '#f7f7f7', borderBottom: '1px solid #e0e0e0',
    ...
    Object.assign(historyWrap.style, {
        ...
        background: '#fff', display: 'flex', flexDirection: 'column', gap: '15px'
    });
    ...
    textInput.type = 'text'; textInput.placeholder = 'Type your question…';
    ...
    Object.assign(sendBtn.style, {
        background: '#D4A017', color: '#000', border: 'none', borderRadius: '8px',
    ...
    const sendMsg = async () =>
    {
        ...
        addMsg(text, 'user');
        textInput.value = '';
        sendBtn.disabled = true; Object.assign(sendBtn.style, { opacity: .7, cursor: 'wait' });
        ...
        try
        {
            ...
            {
                answer = await sendSupportQuery(text);
            } else
            {
        ...
        {
            console.error(err); resolvePendingBot('Something went wrong.');
        } finally
        {
            sendBtn.disabled = false; Object.assign(sendBtn.style, { opacity: 1, cursor: 'pointer' });
        }
    };
    ...
    chatModalRef = {
        modal, history: historyWrap, input: textInput, sendBtn, title, mode: 'question'
    };
}
...
async function onRefreshClick (e)
{
    ...
    btn.disabled = true; Object.assign(btn.style, { opacity: .6, cursor: 'not-allowed' });
    ...
    btn.disabled = false; Object.assign(btn.style, { opacity: 1, cursor: 'pointer' });
    actionsContainer.style.display = 'none';
    mainButton.style.transform = 'rotate(0deg)';
}
...
function addMsg (text, sender, pending = false)
{
    ...
    } else
    {
        ...
        if (pending)
        {
            bubble.classList.add('pending'); bubble.style.fontStyle = 'italic'; bubble.style.opacity = .8;
        }
    }
...
function resolvePendingBot (text)
{
    ...
    if (pending)
    {
        ...
        pending.style.fontStyle = 'normal'; pending.style.opacity = 1;
