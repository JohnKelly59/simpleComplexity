...
export function createSpeedDial ()
{
    ...
    const updateRecordingButtonsVisibility = () =>
    {
        ...
        const hasStoredVideo = state.recordedChunks.length > 0;
        const shouldHideToggles = isRecording || hasStoredVideo;

        // Updated logic for camera and mic buttons
        cameraButton.style.display = shouldHideToggles ? "none" : "flex";
        micButton.style.display = shouldHideToggles ? "none" : "flex";

        // Logic for other buttons remains the same
