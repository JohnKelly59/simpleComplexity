...
import
{
    ...
    SUPPORT_QUERY_ENDPOINT // Import the new endpoint
} from './config.js';
...
import { fetchTooltipsForKeys, fetchWithAuth } from './api.js'; // Import fetchWithAuth
...
// Function to fetch support query response
async function handleSupportQuery (question, panelRef)
{
    ...
    try
    {
        ...
        const data = await response.json(); // Assuming fetchWithAuth resolves with the response object for .json()

        if (data && data.answer)
        {
            panelRef.addMsg(data.answer, 'bot');
            // If provenance exists, you could add it too:
            // if (data.provenance) panelRef.addMsg(`Source: ${data.provenance}`, 'bot-meta');
        } else
        {
...
{ /* Largely unchanged, ensures both chat buttons are present */
    ...
    // Definition Chat Button (Question Assistant)
    ...
    Object.assign(definitionChatButton.style, {
    ...
    definitionChatButton.onmouseenter = () => { if (!definitionChatButton.disabled) definitionChatButton.style.backgroundColor = '#2D3748'; };
    definitionChatButton.onmouseleave = () => { if (!definitionChatButton.disabled) definitionChatButton.style.backgroundColor = '#4A5568'; };
    definitionChatButton.onmousedown = () => definitionChatButton.style.transform = 'scale(0.95)';
    definitionChatButton.onmouseup = () => definitionChatButton.style.transform = 'scale(1)';
    definitionChatButton.innerHTML = SVG_CHAT_ICON; // Original chat icon
    definitionChatButton.addEventListener("click", (event) =>
    {
        event.preventDefault(); event.stopPropagation();
        if (!state.tooltipsEnabled)
        {
            console.log("Tooltips are disabled, cannot open question assistant.");
            return;
        }
        toggleChatModal(true); // Opens the definition chat (existingChatModalRef)
    ...
    Object.assign(supportChatBtn.style, {
        width: "44px", height: "44px", borderRadius: "50%", border: "none", cursor: "pointer",
        backgroundColor: "#4A5568", color: "#FFFFFF", boxShadow: "0 2px 5px rgba(0,0,0,0.25)",
        display: "flex", alignItems: "center", justifyContent: "center",
        transition: 'background-color 0.2s ease, transform 0.1s ease-out'
    });
    supportChatBtn.onmouseenter = () => { if (!supportChatBtn.disabled) supportChatBtn.style.backgroundColor = '#2D3748'; };
    supportChatBtn.onmouseleave = () => { if (!supportChatBtn.disabled) supportChatBtn.style.backgroundColor = '#4A5568'; };
    supportChatBtn.onmousedown = () => supportChatBtn.style.transform = 'scale(0.95)';
    supportChatBtn.onmouseup = () => supportChatBtn.style.transform = 'scale(1)';
    supportChatBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"></path></svg>`; // Support icon
    ...
    mainButton.addEventListener("click", () =>
    {
        ...
        // If tooltips are disabled, main FAB click should only allow opening support chat.
        // If tooltips enabled, it toggles the action menu.
        if (state.tooltipsEnabled)
        {
        ...
        {
            console.log("Tooltips disabled. Main FAB click opens support chat if available.");
            if (!supportChatPanelRef) supportChatPanelRef = createSupportChatPanel();
            if (supportChatPanelRef) supportChatPanelRef.toggle(true);
        }
    });
    ...
    createExistingChatModal(); // For definitions
    if (!supportChatPanelRef) supportChatPanelRef = createSupportChatPanel(); // For support queries
...
{ /* Unchanged */
    if (localSpeedDialRef && localSpeedDialRef.mainButton)
    {
        ...
        localSpeedDialRef.mainButton.title = enabled ? "Tooltip Options" : "Tooltips Disabled (Support Chat Available)";
...
// Renamed existing chat modal functions (for definitions)
...
{ /* Unchanged */
    ...
    { // This is for definition lookup
        ...
        try
        {
            const responseData = await fetchTooltipsForKeys([messageText]); // Uses question-lookup
...
{ // This is for the definition chat
